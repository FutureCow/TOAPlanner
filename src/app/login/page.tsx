...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  CredentialsSignin: 'Ongeldig e-mailadres of wachtwoord.',
...
function LoginContent() {
  ...
  useEffect(() => {
    fetch('/api/settings')
      ...
      .then(d => {
        ...
        if (d.hasCredentials !== undefined) setHasCredentials(d.hasCredentials)
      })
      .catch(() => {})
  }, [])
  ...
  async function handleCredentials(e: React.FormEvent) {
    ...
    if (result?.ok) {
      ...
      setCredError('Ongeldig e-mailadres of wachtwoord.')
    }
  }
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl">
          ...
          {/* Error from URL param (OAuth errors) */}
          {message && (
            ...
            {hasCredentials && (
              <form onSubmit={handleCredentials} className="space-y-2.5">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-semibold py-2.5 px-4 rounded-xl transition-colors text-sm"
                >
